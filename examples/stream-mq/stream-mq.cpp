...
const std::string TERM_SIGNAL = "<TERM>";
...
int main(int argc, char ** argv) {
    ...
    zmq::socket_t audio_socket(context, ZMQ_PULL);
    audio_socket.connect("ipc:///tmp/whisper_audio.sock");  // Connect to receive audio

    // Socket to send transcribed text
    zmq::socket_t text_socket(context, ZMQ_PUSH);
    text_socket.connect("ipc:///tmp/whisper_text.sock");    // Connect to send text
    ...
    fprintf(stderr, "[Ready to receive audio data]\n");
    ...
    while (is_running) {
        ...
        for (; msg_count < max_messages; msg_count++) {
            ...
            auto result = audio_socket.recv(message, zmq::recv_flags::dontwait);
            if (!result.has_value()) {
            ...
            // print result;
            {
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    if (params.no_timestamps) {
                        ...
                        output = "[" + to_timestamp(t0, false) + " --> " + to_timestamp(t1, false) + "]  " + text;
                    ...
                    // Send via ZMQ
                    text_socket.send(zmq::buffer(output.data(), output.size()));
                    
                    // Still print to console/file as before
